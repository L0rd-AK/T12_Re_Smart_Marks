...
// Question Format Controllers
export const getQuestionFormats = async (req: Request, res: Response): Promise<void> => {
    try {
        if (!req.user) {
            res.status(401).json({ message: 'User not authenticated' });
            return;
        }
        const formats = await QuestionFormat.find({ createdBy: req.user._id });
        res.json(formats);
    } catch (error) {
...
export const getQuestionFormat = async (req: Request, res: Response): Promise<void> => {
    try {
        if (!req.user) {
            res.status(401).json({ message: 'User not authenticated' });
            return;
        }
        const format = await QuestionFormat.findOne({
            _id: req.params.id,
            createdBy: req.user._id
        });

        if (!format) {
            res.status(404).json({ message: 'Question format not found' });
            return;
        }
...
export const createQuestionFormat = async (req: Request, res: Response): Promise<void> => {
    try {
        if (!req.user) {
            res.status(401).json({ message: 'User not authenticated' });
            return;
        }
        const format = new QuestionFormat({
            ...req.body,
            createdBy: req.user._id
        });
        ...
        const errorMessage = error instanceof Error ? error.message : 'Unknown error';
        res.status(400).json({ message: 'Error creating question format', error: errorMessage });
    }
};

export const updateQuestionFormat = async (req: Request, res: Response): Promise<void> => {
    try {
        if (!req.user) {
            res.status(401).json({ message: 'User not authenticated' });
            return;
        }
        const format = await QuestionFormat.findOneAndUpdate(
        ...
        const errorMessage = error instanceof Error ? error.message : 'Unknown error';
        res.status(400).json({ message: 'Error updating question format', error: errorMessage });
    }
};

export const deleteQuestionFormat = async (req: Request, res: Response): Promise<void> => {
    try {
        if (!req.user) {
            res.status(401).json({ message: 'User not authenticated' });
            return;
        }
        const format = await QuestionFormat.findOneAndDelete({
            _id: req.params.id,
            createdBy: req.user._id
        });
        ...
        res.json({ message: 'Question format deleted successfully' });
    } catch (error) {
        const errorMessage = error instanceof Error ? error.message : 'Unknown error';
        res.status(500).json({ message: 'Server error', error: errorMessage });
        return;
